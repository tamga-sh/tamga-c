...
> **UNBLOCKED**: `tamga-rust` Sections A–L are implemented, tested, and security-reviewed (see `../tamga-rust/CLAUDE.md`); `../tamga-rust` resolves as a real crate via the path dependency and everything in this repo builds. Not yet done: an actual `cargo publish` of `tamga-rust` to crates.io (no credentials in this environment — `Cargo.toml` documents the swap-over line for when that happens). Sections C (License Checkout FFI), D (Machine Checkout FFI, all 4 signing schemes + HKDF), and E's `tamga_offline_proof_verify` are implemented. `tamga_offline_proof_generate` is a deliberate, documented non-implementation — see the GOTCHAS entry below — not a TODO stub like the earlier sections were. Section F (Memory & Lifecycle security review) is still outstanding. See the plan file for the current per-section checklist.
